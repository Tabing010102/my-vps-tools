<?php
    ...
    $sql = "SELECT `ddl`.*, LEFT(`ddl`.`description`, 20) AS `description_20`, `user`.`name` FROM `ddl` JOIN `user` ON `user`.`uid`=`ddl`.`uid` WHERE `ddl`.`status`=0 AND `ddl`.`deadline`=CURDATE() ORDER BY `ddl`.`priority` DESC, `ddl`.`uid`, `ddl`.`ddlid` DESC";
