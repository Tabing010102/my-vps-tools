<?php
    ...
    $sql = "SELECT `ddl`.*, LEFT(`ddl`.`description`, 20) AS `description_20`, `user`.`name` FROM `ddl` JOIN `user` ON `user`.`uid`=`ddl`.`uid` WHERE `user`.`name`=:un AND `ddl`.`status`=0 AND `ddl`.`deadline`<CURDATE()+$limit ORDER BY `ddl`.`priority` DESC, `ddl`.`deadline`, `ddl`.`lastupdate` DESC, `ddl`.`ddlid` DESC";
