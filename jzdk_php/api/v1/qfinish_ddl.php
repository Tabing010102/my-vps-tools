<?php
    ...
    $sql = "SELECT `user`.`name`, `ddl`.`totalparts` FROM `ddl` JOIN `user` ON `user`.`uid`=`ddl`.`uid` WHERE `ddl`.`ddlid`=:ddlid";
    ...
    if($row_count == 0) {
        ...
        if(strcmp($name, $username) != 0) {
            ...
            $sql = "UPDATE `ddl` SET `completedparts`=:cp, `status`=1 WHERE `ddlid`=:ddlid";
            $dbcp = $dbc->prepare($sql);
